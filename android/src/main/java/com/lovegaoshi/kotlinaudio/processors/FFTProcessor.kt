// credits: https://github.com/dzolnai/ExoVisualizer
...
class FFTAudioProcessor(val sampleRate: Int) : AudioProcessor {
    ...
    interface FFTListener {
        fun onFFTReady(sampleRateHz: Int, channelCount: Int, fft: DoubleArray)
    }
        ...
        while (srcBufferPosition > audioTrackBufferSize) {
            ...
            tempByteArray.forEachIndexed { index, byte ->
                if (currentByte == null) {
                    ...
                    src[index / 2] =
                        ((currentByte * Byte.MAX_VALUE + byte) / (Byte.MAX_VALUE * Byte.MAX_VALUE)).toDouble()
                    dst[index / 2] = 0.0
                    currentByte = null
                }
